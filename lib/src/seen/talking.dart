...
class TalkScreen extends StatefulWidget {
  TalkScreen({Key? key,required this.username,required this.password,required this.userid})
   : super(key: key);
...
class _TalkScreenState extends State<TalkScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      ? null:()async{
          ...
          if(sessionflag==0){
            sessionflag=1;
            int session_time = await sessiontime_get_Http(widget.username);
  ...
  Future<String> _post_request(String text,int session_times,int count) async {
    HttpURL poster = HttpURL();
    Uri url = Uri.parse('http://${poster.hostname}/api/conversations');
    Map<String, String> headers = {'content-type': 'application/json'};
    String body = json.encode(
      {'userid':1,'context':'$text','conversation_times':count,'session_times':session_times,});
